...
function cleanSeoText(value, maxLength = 155) {
...
function renderPage({
  ...
  content,
  active = "",
  description = "Feiermiete ist dein Anbieter für Event-Equipment, Mietartikel, Produktionsküche, Catering-Koordination und Eventservice in Berlin.",
  canonicalPath = "",
  ogImage = "/public/images/equipment-photo.jpg"
}) {
  const siteUrl = "https://www.feiermiete.de";
  const canonicalUrl = `${siteUrl}${canonicalPath || ""}`;
  const pageTitle = title.includes("Feiermiete") ? title : `${title} | Feiermiete`;
  return `
    <!DOCTYPE html>
    <html lang="de">
      <head>
...
export function renderSeoLandingPage({
  ...
  return renderPage({
    title,
    active: "equipment",
    description: cleanSeoText(intro, 155),
    canonicalPath,
    ogImage: image,
    content: `
      <main class="seo-page">
